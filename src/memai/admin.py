...
async def fonts_css(request):
    ...
    The FETCHED file comes FIRST when there is one, and that order is
    load-bearing. "Roboto" names several releases whose advance widths
    differ -- an installed copy measured x at 1030 units where the fetched
    face measures 1016 -- so preferring local() meant the canvas broke a
    node label in a different place on a machine that happened to have
    Roboto installed. diagram_svg.measure() reads a width table extracted
    from the file in webui/fonts/ (tools/gen-roboto-metrics.py), so the
    canvas has to be looking at that same file for the two renderers to
    wrap alike. local() stays as the fallback for an unpopulated
    checkout, where nothing can be guaranteed anyway.
    """
