"""Carrying memories from one store to another.
...
def test_a_dry_run_reports_the_slice_and_moves_nothing(home):
    with db.connect(store="general") as conn:
        ids = _seed(conn)
        edits = conn.execute(
...
def test_a_move_carries_everything_and_then_removes_the_originals(home):
    ...
    with db.connect(store="acme") as dst:
    ...
    with db.connect(store="general") as src:
...
def test_create_makes_the_target_on_the_real_run_only(home):
    ...
    assert plan["creates"] is True and not db.store_exists("zeta")
    result = portable.move("general", "zeta", domain="acme/x100", create=True, dry_run=False)
    assert result["moved"] == 3 and db.store_exists("zeta")
...
def test_the_dashboard_moves_a_selection_after_a_dry_run(client, home):
    with db.connect() as conn:
        ids = _seed(conn)
    plan = client.post("/api/stores/move", json={"target": "acme", "uids": [ids["note"]]}).json()
    assert plan["dry_run"] is True and plan["memories"] == 1
    assert _count("acme") == 0
    done = client.post("/api/stores/move",
                       json={"target": "acme", "uids": [ids["note"]], "dry_run": False}).json()
    assert done["moved"] == 1
    assert _count("acme") == 1 and _count("general") == 3
    same = client.post("/api/stores/move", json={"target": "general", "uids": [ids["old"]]})
    assert same.status_code == 400


def test_the_mcp_tool_moves_out_of_the_active_store(home):
    with db.connect() as conn:
        ids = _seed(conn)
    plan = server.move_to_store("acme", uids=f"{ids['note']}, {ids['old']}")
    assert plan["dry_run"] is True and plan["source"] == "general" and plan["memories"] == 2
    result = server.move_to_store("acme", domain="acme/x100", dry_run=False)
    assert result["moved"] == 3
    # active rows: the archived `old` is not one of them
    assert {s["name"]: s["memories"] for s in server.list_stores()["stores"]} == {
        "general": 1, "acme": 2}
