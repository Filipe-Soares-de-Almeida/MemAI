...
def test_counts_accumulate(store):
    uid = server.note("fixture title", content="the queue drain is single threaded")["uid"]
    for _ in range(3):
...
def test_listing_counts(store):
    uid = server.note("fixture title", content="an empty part is rejected", domain="acme/x100")["uid"]
...
def test_the_curation_corpus_reports_usage(store):
    used = server.note("fixture title", content="cache warmup runs nightly")["uid"]
    unused = server.note("fixture title", content="the digest mailout follows the audit sweep")["uid"]
    server.search("cache warmup")
...
def test_never_recalled_counts_the_whole_corpus_not_the_page(store):
    for i in range(5):
        server.note("fixture title", content=f"finding {i}: the export window is inclusive on both ends")
...
def test_an_unknown_sort_falls_back_instead_of_reaching_sql(client):
    client.post("/api/memories", json={"title": "fixture title", "type": "note", "content": "x"})
